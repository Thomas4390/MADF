...
class CrossValidationData:
    ...
    def __next__(self) -> Tuple[pd.DataFrame, Dict[Union[0, 1], pd.DataFrame]]:
        ...
        testDataToReturn = {0: self.testData[self.nextFold][0], 1: self.testData[self.nextFold][1]}
...
def crossValidationSplitForTimeSeries(data: pd.DataFrame, numberFolds: int = 5, cushion: int = 0) -> CrossValidationData:
    '''
    ...
    for fold in range(numberFolds):
        testData[fold] = {}
        trainData[fold] = data.iloc[(fold * trainLength):((fold + 1) * trainLength), :]
