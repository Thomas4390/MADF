...
newVariableDataFrame, newVariableToTradeDataFrame, indicatorsDataFrame = getData(rollingWindow=60,
                                                                                 numberOfPairsToTrade=2,
                                                                                 method='alphaFactor')

y = (newVariableDataFrame > newVariableDataFrame.shift(1)).shift(-1)  # on veut prédire le lendemain
...
fpr, tpr, thresholds = metrics.roc_curve(y_test.to_numpy(), modForestClass.predict(X_test.to_numpy().reshape(-1, 1)), pos_label=1)
